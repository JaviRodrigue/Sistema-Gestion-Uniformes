...
public class ObtenerClientePorDniCasoDeUso(IClienteRepository repo)
{
    ...
    public async Task<BuscarClienteDTO> EjecutarAsync(string dni)
    {
        var cliente = await _repository.ObtenerClientePorDni(dni)
            ?? throw new Exception("No se encontró el cliente, dni inexistente");
